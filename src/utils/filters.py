...
def convert_flat_index_to_grid_index(flat_index: int, param_shapes: np.ndarray) -> tuple:
    """
    Convert flattened index to multi-dimensional grid indices.
    
    Args:
        flat_index: Linear index in flattened array
        param_shapes: Array of parameter dimensions
        
    Returns:
        Tuple of grid indices
        
    Example:
        >>> param_shapes = np.array([3, 2, 4])  # 3x2x4 grid
        >>> convert_flat_index_to_grid_index(10, param_shapes)
        (1, 1, 0)
    """
    indices = []
    remaining = flat_index
    
    for i in range(len(param_shapes) - 1, -1, -1):
        indices.insert(0, remaining % param_shapes[i])
        remaining //= param_shapes[i]
    
    return tuple(indices)


def convert_grid_index_to_flat_index(grid_indices: tuple, param_shapes: np.ndarray) -> int:
    """
    Convert multi-dimensional grid indices to flattened index.
    
    Args:
        grid_indices: Tuple of grid indices
        param_shapes: Array of parameter dimensions
    ...
    Example:
        >>> param_shapes = np.array([3, 2, 4])  # 3x2x4 grid
        >>> convert_grid_index_to_flat_index((1, 1, 0), param_shapes)
        10
    """
    flat_index = 0
    multiplier = 1
    ...
    for i in range(len(param_shapes) - 1, -1, -1):
        flat_index += grid_indices[i] * multiplier
        multiplier *= param_shapes[i]
