...
def sample_dataframe():
    ...
    return pd.DataFrame({
        ...
        'unrealized_gains': [1e8, 1.5e8, 2e8, 2.5e8, 3e8]
    })
...
def sample_dataframe_with_nan():
    ...
    return pd.DataFrame({
        ...
        'unrealized_gains': [1e8, 1.5e8, 2e8, np.nan, 3e8]
    })
...
def sample_h5_file(temp_dir):
    ...
    with h5py.File(h5_path, 'w') as f:
        ...
        f.create_dataset('unrealized_gains', data=[1e8, 1.5e8, 2e8, 2.5e8, 3e8])
...
def postprocess_config_yaml(temp_dir):
    ...
    config = {
        ...
        'output_filters': {
            ...
            'unrealized_gains': {'min': 0, 'max': 1e10}
        },
