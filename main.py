def main():
    ...
    # Import all ParameterField variables from config_module into local scope
    V_plasma_field = getattr(config_module, 'V_plasma_field', None)
    T_i_field = getattr(config_module, 'T_i_field', None)
    n_tot_field = getattr(config_module, 'n_tot_field', None)
    tau_p_T_field = getattr(config_module, 'tau_p_T_field', None)
    tau_p_He3_field = getattr(config_module, 'tau_p_He3_field', None)
    P_aux_field = getattr(config_module, 'P_aux_field', None)
    P_aux_DT_eq_field = getattr(config_module, 'P_aux_DT_eq_field', None)
    TBR_DT_field = getattr(config_module, 'TBR_DT_field', None)
    TBR_DDn_field = getattr(config_module, 'TBR_DDn_field', None)
    tau_ifc_field = getattr(config_module, 'tau_ifc_field', None)
    tau_ofc_field = getattr(config_module, 'tau_ofc_field', None)
    eta_th_field = getattr(config_module, 'eta_th_field', None)
    capacity_factor_field = getattr(config_module, 'capacity_factor_field', None)
    cost_of_electricity_field = getattr(config_module, 'cost_of_electricity_field', None)
    I_target_field = getattr(config_module, 'I_target_field', None)
    total_time = getattr(config_module, 'total_time', None)
    ...
    if analysis_type == 'T_seeded':
        ...
        exit()
    # ======================== SYSTEM PROFILER ========================
    # --- Performance optimized parallel settings ---
    n_jobs = os.cpu_count() or 4  # maximize n_jobs
    chunk_size = max(2000, (os.cpu_count() or 4) * 500)  # large chunk size
    batch_size = 500
    N_SAMPLES = 100000
    order = 3
    print(f"[Optimized parallel] n_jobs={n_jobs}, chunk_size={chunk_size}, batch_size={batch_size}, N_SAMPLES={N_SAMPLES}, order={order}")
    ...
    with h5py.File(output_filename, 'w') as h5_file:
        ...
        vector_fields = ['N_ofc', 'N_ifc', 'N_stor', 'n_T', 'n_D', 'P_DDn', 'P_DDp', 'P_DT', 'TBE']  # Updated to match new output keys
        ...
        overall_pbar = tqdm(total=n_combinations, desc="Total computation", unit="comb")

        # --- Prime Numba compilation ---
        if analysis_type == 'T_seeded':
            compute_single_combination(0, input_arrays_flat, param_shapes_array, total_time, vector_length)

        # --- Parallel computation with joblib ---
        buffer_results = []
        buffer_indices = []
        buffer_size = 1000  # batch HDF5 writes every 1000 results
        for chunk_start in range(0, n_combinations, chunk_size):
            ...
            if analysis_type == 'lump':
                ...
                results = Parallel(n_jobs=n_jobs, backend='loky')(
                    delayed(compute_single_combination)(*args) for args in task_args
                )
            ...
            for i, result in enumerate(chunk_results):
                ...
                # Only update progress bar here
                if i % 10 == 0:
                    overall_pbar.update(10)
                # Batch write if buffer is full
                if len(buffer_results) >= buffer_size:
                    for buf_result, buf_idx in zip(buffer_results, buffer_indices):
                        for field in data_fields:
                            value = buf_result.get(field, None)
                            # --- For each result, save all its fields (inputs, outputs) into the correct HDF5 datasets.
                            if field == 'error':
                                if value is None or (isinstance(value, float) and not np.isfinite(value)):
                                    datasets[field][buf_idx] = ""
                                else:
                                    datasets[field][buf_idx] = str(value)
                            elif field == 'sol_success':
                                datasets[field][buf_idx] = bool(value)
                                ...
                                if arr.ndim == 0 or arr.size == 0:
                                    ...
                                    datasets[field][buf_idx, :] = padded
                                elif arr.shape[0] == vector_length:
                                    datasets[field][buf_idx, :] = arr
                                else:
                                    ...
                                    if 'error' in datasets:
                                        datasets['error'][buf_idx] += f"Warning: {field} length {arr.size} != expected {vector_length}, padded with NaN"
                            else:
                                if value is None or (isinstance(value, float) and not np.isfinite(value)):
                                    datasets[field][buf_idx] = np.inf
                                elif isinstance(value, str):
                                    datasets[field][buf_idx] = np.nan
                                    ...
                                    datasets[field][buf_idx] = scalar_value if np.isfinite(scalar_value) else np.inf
                                else:
                                    try:
                                        scalar_value = float(value)
                                    except Exception:
            ...
            # If not a multiple of 10, update remaining progress
        ...
        # Final batch write for any remaining results
        if buffer_results:
            for buf_result, buf_idx in zip(buffer_results, buffer_indices):
                for field in data_fields:
                    value = buf_result.get(field, None)
...
if __name__ == "__main__":
    ...
    # --- Profile compute_single_combination for T_seeded ---
    from utils.Tseeded_functions import compute_single_combination
    # Prepare representative arguments for profiling
    # Use first combination in parameter grid
    input_file_name = "config_test"
    config_module = importlib.import_module(f"inputs.{input_file_name}")
    V_plasma_field = getattr(config_module, 'V_plasma_field', None)
    T_i_field = getattr(config_module, 'T_i_field', None)
    n_tot_field = getattr(config_module, 'n_tot_field', None)
    ...
    input_arrays_flat = [
        V_plasma_field.data.to('m^3').magnitude.flatten(),
        T_i_field.data.to('keV').magnitude.flatten(),
        n_tot_field.data.to('1/m^3').magnitude.flatten(),
        tau_p_T_field.data.to('s').magnitude.flatten(),
        P_aux_field.data.to('W').magnitude.flatten(),
        P_aux_DT_eq_field.data.to('W').magnitude.flatten(),
        TBR_DT_field.data.to_base_units().magnitude.flatten(),
        TBR_DDn_field.data.to_base_units().magnitude.flatten(),
        tau_ifc_field.data.to('s').magnitude.flatten(),
        tau_ofc_field.data.to('s').magnitude.flatten(),
        eta_th_field.data.to_base_units().magnitude.flatten(),
        capacity_factor_field.data.to_base_units().magnitude.flatten(),
        cost_of_electricity_field.data.to('1/J').magnitude.flatten()
    ]
    param_shapes_array = np.array([arr.shape[0] for arr in input_arrays_flat], dtype=np.int64)
    vector_length = 100
    ...
    compute_single_combination(0, input_arrays_flat, param_shapes_array, total_time, vector_length)
    profiler2.disable()
    stats2 = pstats.Stats(profiler2).sort_stats('cumtime')
    print("\n--- Profiling compute_single_combination (T_seeded, first combination) ---")
